...
use App\Http\Resources\API\ProviderServiceResource;
...
class UserController extends Controller
{
    ...
    public function verify(Request $request)
    {
     ...
     public function legal_document(Request $request){
         
         $user_data = User::find($request->id);
         
          if(isset($request->profile_image) && $request->profile_image != null ) {
            $user_data->clearMediaCollection('profile_image');
            $user_data->addMediaFromRequest('profile_image')->toMediaCollection('profile_image');
        }
        
         if(isset($request->aadhar_image) && $request->aadhar_image != null ) {
            $user_data->clearMediaCollection('aadhar_image');
            $user_data->addMediaFromRequest('aadhar_image')->toMediaCollection('aadhar_image');
        }
        
         if(isset($request->local_address) && $request->local_address != null ) {
            $user_data->clearMediaCollection('local_address');
            $user_data->addMediaFromRequest('local_address')->toMediaCollection('local_address');
        }
        $message = __('messages.updated');
        $user_data['profile_image'] = getSingleMedia($user_data,'profile_image',null);
     ...
     public function providerList(Request $request){
        ...
        $rating_data = ProviderService::where('service_id',$request->service_id);
        ...
        $items = ProviderServiceResource::collection($rating_data);
        $response = [
            'pagination' => [
                'total_items' => $items->total(),
                'per_page' => $items->perPage(),
    ...
    public function work_experience_submit(Request $request){
